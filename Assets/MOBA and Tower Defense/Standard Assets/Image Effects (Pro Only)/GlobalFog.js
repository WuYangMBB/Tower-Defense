...
class GlobalFog extends PostEffectsBase {
	...
	function OnRenderImage (source : RenderTexture, destination : RenderTexture) {	
		...
		var toRight : Vector3 = GetComponent.<UnityEngine.Camera>().transform.right * CAMERA_NEAR * Mathf.Tan (fovWHalf * Mathf.Deg2Rad) * CAMERA_ASPECT_RATIO;
		var toTop : Vector3 = GetComponent.<UnityEngine.Camera>().transform.up * CAMERA_NEAR * Mathf.Tan (fovWHalf * Mathf.Deg2Rad);
	
		var topLeft : Vector3 = (GetComponent.<UnityEngine.Camera>().transform.forward * CAMERA_NEAR - toRight + toTop);
		var CAMERA_SCALE : float = topLeft.magnitude * CAMERA_FAR/CAMERA_NEAR;	
			
		topLeft.Normalize();
		topLeft *= CAMERA_SCALE;
	
		var topRight : Vector3 = (GetComponent.<UnityEngine.Camera>().transform.forward * CAMERA_NEAR + toRight + toTop);
		topRight.Normalize();
		topRight *= CAMERA_SCALE;
		
		var bottomRight : Vector3 = (GetComponent.<UnityEngine.Camera>().transform.forward * CAMERA_NEAR + toRight - toTop);
		bottomRight.Normalize();
		bottomRight *= CAMERA_SCALE;
		
		var bottomLeft : Vector3 = (GetComponent.<UnityEngine.Camera>().transform.forward * CAMERA_NEAR - toRight - toTop);

...
public class CameraMotionBlur extends PostEffectsBase 
{
	...
	private function CalculateViewProjection() {
		var viewMat : Matrix4x4 = GetComponent.<UnityEngine.Camera>().worldToCameraMatrix;
		var projMat : Matrix4x4 = GL.GetGPUProjectionMatrix (GetComponent.<UnityEngine.Camera>().projectionMatrix, true);
	...
	function OnRenderImage (source : RenderTexture, destination : RenderTexture) {	
		...
		if (filterType == MotionBlurFilter.CameraMotion)
		{
			...
			// pitch (vertical)
			farHeur = (Vector3.Angle (transform.up, prevFrameUp) / GetComponent.<UnityEngine.Camera>().fieldOfView) * (source.width * 0.75f);
			...
			farHeur = (Vector3.Angle (transform.forward, prevFrameForward) / GetComponent.<UnityEngine.Camera>().fieldOfView) * (source.width * 0.75f);
			...
			if (preview) // crude approximation
				motionBlurMaterial.SetVector ("_BlurDirectionPacked", Vector4 (previewScale.y, previewScale.x, 0.0f, previewScale.z) * 0.5f * GetComponent.<UnityEngine.Camera>().fieldOfView);
			else
	...
	function GetTmpCam () : Camera {
		if (tmpCam == null) {
			var name : String = "_" + GetComponent.<UnityEngine.Camera>().name + "_MotionBlurTmpCam";
		...
		tmpCam.GetComponent.<UnityEngine.Camera>().depthTextureMode = DepthTextureMode.None;
		tmpCam.GetComponent.<UnityEngine.Camera>().clearFlags = CameraClearFlags.Nothing;
